...
class playerActivity : AppCompatActivity() {
    ...
    companion object{ //--------------------------------------------------------------------------------------------------------------------------------------------
        ...
        fun setupRecycler(animation:Boolean = false,justUpdate:Boolean=false) {
            if(justUpdate){
                // Set up the Adapter
                if(animation){
                ...
                adapter.notifyItemChanged(data.queueSelectedSong)
            }else{
                // Find the RecyclerView in your layout
                recyclerView = b.root.findViewById(R.id.recyclerViewPlayer)
                ...
                // Set up the Adapter
                adapter = PlayerAdapter(data.queue, frag_manager)
                ...
                // Add OnScrollListener to detect scrolling
                recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                    override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                        super.onScrollStateChanged(recyclerView, newState)

                        if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                            // When scrolling stops, get the current snapped position
                            val snapView = (recyclerView.layoutManager as LinearLayoutManager)
                                .findViewByPosition(
